...
const SavedCommentsPage = async () => {
    ...
    if (!user) return (
        <section className="size-screen">
            <ShowError heading="Oops! We could'nt find anything" messages={["Looks like you're not logged in", "Log-in to see your Saved Comments here"]} />
        </section>
    )
    ...
    await queryClient.prefetchInfiniteQuery({
        queryFn: () => queryFunction(getSavedContent, [user_id, "comment", 1], 1),
        queryKey: getQueryKeys(`saved_comments_uid`, { uid: user_id }),
        initialPageParam: 1,
        staleTime: 3600 * 1000,
        gcTime: 3600 * 1000,
    })
