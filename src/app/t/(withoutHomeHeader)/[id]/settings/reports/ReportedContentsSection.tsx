...
import ReportedContentBar from "@components/ReportedContentBar";
import { LoadingSpinner } from "@components/ui";
import { actionOnReportedContents } from "@lib/helpers/client";
...
const ReportedContentsSection = ({ tid, type, uid }: Props) => {
    ...
    const handleDecision = (id: string, action: AvailableActionsForReport) => {
        if (decisionBuffer.size >= 50)
            toast.error("Only 50 decisions are allowed to save at a time")
    ...
    const handleSave = () => {
        if (decisionBuffer.size < 1 || decisionBuffer.size > 50)
            toast.error("At least 1 upto 50 decisions are allowed to save at a time.")
