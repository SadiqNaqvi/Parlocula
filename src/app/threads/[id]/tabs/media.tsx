...
import { FrameTile, LoadingSpinner, NotFound } from "@components/ui";
...
const fetchInitialData = async (id: string, page: number): Promise<{ result: any[], total: number } | null> => {
    try {
        const { result, success }: GeneralMultipleReturn = await fetch(`/api/threads/${id}/frames?p=${page}`, {
            next: {
                revalidate: 0,
                tags: [`frames-of-thread:${id}`],
            }
        }).then(res => res.json());
...
const PostsTab = async ({ id, page = 1 }: { id: string, page?: number }) => {
    ...
    if (response?.total && !response?.result.length) return <NotFound title="You've come across too far" paragraph={["Please go back and try again."]} />
    ...
    const fetchData = async (page = 1): Promise<InfiniteQueryResponse> => {
        const { error, result, success }: GeneralMultipleReturn = await fetch(`/api/threads/${id}/frames?p=${page}`, {
            next: {
                revalidate: 0,
                tags: [`frames-of-thread:${id}`],
            }
