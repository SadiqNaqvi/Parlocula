...
import { FrameDataSchemaType } from "@lib/schemas";
...
import { deleteMultipleMedia, mediaUploader } from "./serverActions";
...
export const getRequest = (
  ...
  precheck?: (
    req: NextRequest,
    params?: any
  ) => Omit<GeneralGetReturn, "result">
) => {
  return async function (req: NextRequest, { params }: { params?: any }) {
    try {
      if (precheck) {
        const { errCode, success } = await precheck(req, params);
        if (!success)
          return NextResponse.json({ result: null, success, errCode });
      }
...
export const ppGetData = async <T = GeneralGetReturn>({
  url,
  revalidate,
  tag,
  options,
...
const uploadFiles = async ({
  ...
  return filesData
    .map((frame) => {
      if (frame.isExternal) return { path: frame.url, type: frame.type };
      const url = results.shift();
      if (url) return { path: url, type: frame.type };
      else return false;
    })
...
export const postRequest = ({
  ...
  return async function (req: NextRequest, { params }: { params: any }) {
    ...
    const user_id = cookieStore.get("uid")?.value;
    const username = cookieStore.get("username")?.value;
    if (
      !(
        req.url.includes("user/register") ||
        req.url.includes("user/login") ||
        req.url.includes("api/v1/mediaItem")
      ) &&
      !(user_id && username)
    )
      return NextResponse.json({
...
export const deleteRequest = (
  ...
  return async (req: NextRequest, { params }: any) => {
    ...
    const username = cookieStore.get("username")?.value;
    if (!user_id || !username)
      return NextResponse.json({ success: false, errCode: "pp201" });
...
export const getPostsOfThread = async (
  ...
  await ppGetData<GeneralMultipleReturn>({
...
export const getFramesOfThread = async (
  tid: string,
  page: number,
  filter: string = "latest"
): Promise<GeneralMultipleReturn> =>
  await ppGetData({
    url: `post/thread/${tid}/frames?p=${page}&f=${filter || "latest"}`,
    revalidate: oneDay,
    tag: "filteredFramesOfThread_filter_tid_page",
    options: { filter: filter || queryFilters.posts[0], tid, page },
  });

export const getLinksOfThread = async (
  tid: string,
  page: number,
  filter: string = "latest"
): Promise<GeneralMultipleReturn> =>
  await ppGetData({
    url: `post/thread/${tid}/links?p=${page}&f=${filter || "latest"}`,
    revalidate: oneDay,
    tag: "filteredLinksOfThread_filter_tid_page",
    options: { filter: filter || queryFilters.posts[0], tid, page },
  });
