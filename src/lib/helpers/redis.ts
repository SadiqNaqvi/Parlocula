...
const removeRoomFromList = async (user_id: string, rmid: string, pipeline?: ChainableCommander) => {
    const transaction = pipeline ?? (await getRedis()).multi();
...
const addMessageInList = async ({ items, room_id, total, pipeline }: { pipeline?: ChainableCommander, room_id: string, items: ItemParamForSortedSet[], total?: number }) => {
    ...
    transaction.zremrangebyrank(`room:${room_id}:messages`, 0, -(queryLimit * 2));
...
const pushParticipantInList = async (room_id: string, items: { uid: string, type: ParticipantEnumType }[], pipeline?: ChainableCommander) => {
    ...
    items.forEach(item => {
        ...
        else transaction.sadd(`room:${room_id}:participants`, `${item.uid}-${item.type}`);
    });
...
export const getParticipantsOfRoom = async (room_id: string, pipeline?: ChainableCommander) => {
    return (pipeline ?? await getRedis()).smembers(`room:${room_id}:participants`);
}
...
const setParticipant = async (room_id: string, user_id: string, participant: Partial<CachedParticipantType>, pipeline?: ChainableCommander) => {
...
const removeParticipant = async (room_id: string, user_id: string, type: ParticipantEnumType | "unknown", pipeline?: ChainableCommander) => {
    ...
    transaction.srem(`room:${room_id}:participants`, `${user_id}-${type}`);
...
const deleteRoom = async (rmid: string, pipeline?: ChainableCommander) => {
    const transaction = pipeline ?? (await getRedis()).multi();
    const participants = await getParticipantsOfRoom(rmid) as string[];

    participants.forEach(participant => {
        const [uid, type] = participant.split('-');
        removeParticipant(rmid, uid, type as ParticipantEnumType)
...
export const addParticipantInRoom = async (uid: string, rmid: string) => {
    ...
    setParticipant(rmid, uid, { participantType: "participant", seenAt: Date.now(), lastSync: Date.now() }, pipeline);
...
export const removeRoomFromInvitation = async (uid: string, rmid: string, removeRoom: boolean) => {
    const redis = await getRedis();
    const pipeline = redis.multi();
    removeParticipant(rmid, uid, "unknown", pipeline);
    removeInvitationFromList(uid, rmid, pipeline);
    if (removeRoom) await deleteRoom(rmid, pipeline);
    await pipeline.exec().then(handlePipelineResponse);
}
...
const createPipelineForRoomList = (user_id: string, page: number, invitation?: boolean) => {
    ...
    const p = Boolean(invitation) ? 0 : page - 1;

    const stage: Stage<{ data: MereRoomType[], total: number }>[] = [
        { method: "zrevrange", key, ref: "roomList", options: [p * queryLimit, ((p + 1) * queryLimit) - 1] },
        ...
        { method: "execute" },
        {
            method: "return", value: (store) => {
                ...
                const data: MereRoomType[] = roomList.map(room_id => {
                    const { invitationMessage, lastMessage, lastMessageAt, lastMessageBy, name, poster, type, } = store.get(`room:${room_id}`) as CachedFullRoomType;
                    const { mute, participantType, seenAt } = store.get(`room:${room_id}:participant:${user_id}`) as ParticipantType;
                    ...
                    if (type === "private" && !(otherParticipantMetaData || otherParticipantMetaData)) throw new Error("Participant data missing");
                    return {
                        ...
                        room_id, invitationMessage, lastMessage, lastMessageAt, lastMessageBy, mute, otherParticipant_id, seenAt
                    }
                });
...
export const storeInvitationInCache = async (user_id: string, list: { data: RoomListResponse[], total: number }) => {
    ...
    list.data.forEach(room => {
        const { room_id, display_name, invitationMessage, lastMessage, lastMessageAt, lastMessageBy, otherParticipant_id, poster, room_type } = room;
...
export const getRoomDetails = async (room_id: string, user_id: string): Promise<FullRoomType | null> => {
    try {
        ...
        const { invitationMessage, lastMessage, lastMessageAt, lastMessageBy, name, poster, type } = room;
            ...
            { method: "execute" },
            {
                method: "return", value: (store) => {
                    ...
                    return {
                        ...
                        users: type === "private" ? [user_id, participantMeta._id] : [],
...
export const setDataWhileCreatingRoom = async (room: RoomModelType, creator: string, participants: ParticipantModelType[]) => {
    try {
        const redis = await getRedis();
        ...
        participants.forEach(participant => {
            ...
            setParticipant(room_id, user_id, { participantType: type, seenAt, mute, hideAt }, pipeline);
...
export const getMessagesFromCache = async (room_id: string, page: number) => {
    try {
        const redis = await getRedis();
        const pipeline = redis.multi();

        pipeline.zrange(`room:${room_id}:messages`, page * queryLimit, ((page + 1) * queryLimit) - 1);
        pipeline.get(`room:${room_id}:messages:total`);

        const [messageIds, total] = await pipeline.exec().then(resp => {
            if (!resp) throw new Error("No response found using pipeline while fetching messages for a room.")

            return resp.map((res, i) => {
                const [err, result] = res;
                if (err) throw new Error(`Got error while fetching messages: ${err.message}`)
                else if (i === 0) {
                    return (result as string[]).map(msg => JSON.parse(msg ?? "null")).filter(Boolean);
                } else return JSON.parse(result as string);
            })
        }) as [MessageItem[], number];

        if (!messageIds.length) return null;

        const messages: MessageModelType[] = await redis
            .mget(messageIds.map(({ msg_id }) => `message:${msg_id}`))
            .then(msgs => msgs
                .map(msg => JSON.parse(msg ?? "null"))
                .filter(Boolean)
            )
...
const storeNewMessageInList = async (room_id: string, user_id: string, message: MessageModelType) => {
...
export const handleNewMessage = async (room_id: string, user_id: string, message: MessageModelType, session: ClientSession) => {
    ...
    await storeNewMessageInList(room_id, user_id, message);
}
