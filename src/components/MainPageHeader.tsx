...
export default function MainPageHeader({ content }: { content: RefinedMovieData }) {
    ...
    const mediaForList = {
        ...
        year: content.year,
        media_type: "movie" as "movie", // Make sure to correct this when this component is used in other pages
        isConfirm: true,
            ...
            <section className="-mt-4 bg-[var(--primary)] md:mt-0 rounded-t-2xl mx-auto max-w-screen-lg relative px-2 lg:px-0">
                ...
                <div className="mt-4 text-sm flex gap-2">
                    ...
                    {new Date(content.release_date) < new Date() &&
                        <button onClick={() => router.push(`${pathname}?action=rate`, { scroll: false })} className="px-4 py-1 border border-gray30 rounded-md">Already Watched?</button>
                    }
                    <AddToList className="flex gap-3 px-4 py-1 items-center border border-gray30 rounded-md" media={mediaForList} />
                </div>
            </section>
        </>
    )
