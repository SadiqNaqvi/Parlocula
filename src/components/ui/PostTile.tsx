...
import { getInternalPoster, numberConverter, timeAgo, refineString } from "@lib/utils";
...
export default function PostTile({ className = "", _id, comment_count, nsfw, createdAt, editedAt, poster, reaction_count, spoiler, tag, thread_id, title, frames, saved_count, username, name }: { className?: string } & MerePost) {
    ...
    return (
        <article className="space-y-3 border-b border-gray40 py-3">
            ...
            <Navigate role="button" comp="link" goto={`/p/${_id}-${refineString(title)}`} className={`w-full ${className}`}>
