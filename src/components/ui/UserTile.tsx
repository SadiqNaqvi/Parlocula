    ...
    return (
        <Navigate comp="link" goto={`/u/${username}`}>
            <article className="flex items-center gap-3 py-2 px-4 border border-gray30 rounded-md">
                <Image className="size-[50px] object-cover" height={50} width={50} alt={`Profile picture of ${username}`} src={getPoster({ path: profile, options: { height: "50", width: "50", crop: "fill" } })} />
